...
def userland(): 
    modules_userland_defect = """

        linux-vdso.so.1 (0x00007ffcbf3ec000)
        libselinux.so.1 => /lib/x86_64-linux-gnu/libselinux.so.1 (0x0000752218207000)
        libcap.so.2 => /lib/x86_64-linux-gnu/libcap.so.2 (0x00007522181fb000)
        libc.so.6 => /lib/x86_64-linux-gnu/libc.so.6 (0x0000752218005000)
        libpcre2-8.so.0 => /lib/x86_64-linux-gnu/libpcre2-8.so.0 (0x0000752217f56000)
        /lib64/ld-linux-x86-64.so.2 (0x000075221826a000)



"""
    print(modules_userland_defect)
    
    tip = """
Check your modules using: ldd /bin/ls and make sure there are no others.

If there is another one different from this, it will be removed (possible rootkit)"""
    print(tip)
    path = input("PATH (without the file): ")
    name_file = input("FILE NAME: ")
    try: 
        os.system("echo > /etc/ld.so.preload")
        print("[*] /etc/ld.so.preload clean")
        time.sleep(1)
        os.system(f"rm -r {path}/{name_file}")
        print("[*] The suspicious file has been deleted.")
...
def userland_krux(): 
    ...
    try: 
        # Delete userland rootkit in Linux 
        answer = input("[*] Do you want delete the userland rootkits?: ")
        if answer == "Yes": 
            userland()
        else: 
...
"""
    ...
    # Use of ModTracer for delete and check for LKMs
    try: 
        answer = input("Do you want to load the ModTracer module for LKM rootkit detection?: ")
        if answer == "Yes":
            os.system("insmod modtracer.ko")
            time.sleep(1)
            print("Module loaded successfully, run 'dmesg'.")
        else: 
            print("OPERATION FAILED")
    except Exception as e: 
        print(f"Error: {e}")
